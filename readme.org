...
*** Kalman filter
The Kalman filter is actually a state observer of the
LTI-system.
...
uncertainty. And in the "correct" (or "update") step, the
measurement of the output vector is obtained and it is used
to update the predicted state vector and its uncertainty.

After given an inital estimation of the state vector and its
uncertainty, the Kalman filter is able to make its first
...
explanation of how kalman filter works:
...
|-------+----------+--------------------------------------------------------------------------------------------|
|   2   |    1     | get the measured system output [[file:figures/z1.png]] and its uncertainty [[file:figures/R1.png]] |
|   3   |    1     |   /update:/ update current state [[file:figures/x11.png]] and uncertainty [[file:figures/P11.png]]   |
|   4   |    1     |         /predict:/ make predictions of [[file:figures/x21.png]] and [[file:figures/P21.png]]         |
|-------+----------+--------------------------------------------------------------------------------------------|
|   5   |    2     | get the measured system output [[file:figures/z2.png]] and its uncertainty [[file:figures/R2.png]] |
|   6   |    2     |   /update:/ update current state [[file:figures/x22.png]] and uncertainty [[file:figures/P22.png]]   |
|   7   |    2     |         /predict:/ make predictions of [[file:figures/x32.png]] and [[file:figures/P32.png]]         |
|-------+----------+--------------------------------------------------------------------------------------------|
|  ...  |   ...    |                                            ...                                             |
|-------+----------+--------------------------------------------------------------------------------------------|
| 3n-1  |    n     | get the measured system output [[file:figures/zn.png]] and its uncertainty [[file:figures/Rn.png]] |
|  3n   |    n     |   /update:/ update current state [[file:figures/xnn.png]] and uncertainty [[file:figures/Pnn.png]]   |
...
~Kalman~ instance. These three matrices can also be defined or
...
*** Predict and update
After setting the system matrices and initial values, the
...
called with control input ~u~ and its covariance matrix ~Q~:
 ...
 #+end_src
If the system has no control input, ~u~ can be set to ~None~. If ~Q~ is not given, the latest set value for ~Q~ will be used.
...
#+end_src
If ~R~ is not given, its latest set value will be used.
